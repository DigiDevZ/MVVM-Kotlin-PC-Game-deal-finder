...
class SearchResultFragment : Fragment(R.layout.fragment_search_result),
    ...
    // Since _binding var can be nullable and to avoid having to use the safe operator through out this fragment
    // I have set up the binding val to only be equal to _binding when it is not null.
    // When this fragment is destroyed, specifically onDestroyView, set the _binding to be null.
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val gamesAdapter = GameListAdapter(this)
        ...
        binding.apply {
            ...
            rvGames.adapter = gamesAdapter.withLoadStateHeaderAndFooter(
                header = GameListLoadStateAdapter { gamesAdapter.retry() },
                footer = GameListLoadStateAdapter { gamesAdapter.retry() }
            )
        ...
        gamesAdapter.addLoadStateListener { loadState ->
            binding.apply {
                ...
                if (loadState.source.refresh is LoadState.NotLoading &&
                    loadState.append.endOfPaginationReached && gamesAdapter.itemCount < 1
                ) {
    ...
    override fun onItemClick(game: GameListLookupResponse) {
        ...
        val action = SearchResultFragmentDirections.actionSearchResultFragmentToGameDetailsFragment(
            game = game,
            ...
            override fun onQueryTextSubmit(query: String?): Boolean {
                if (query != null) {
                    ...
                    if (!ValidationUtil.validateSearchQuery(query)) {
                        displayToast(
                            getString(R.string.search_empty_search_field),
                            Toast.LENGTH_SHORT
                        )
    ...
    private fun displayToast(_text: String, _duration: Int) {
        Log.i("SRFragment.TAG", "displayToast: Toast displaying.")
